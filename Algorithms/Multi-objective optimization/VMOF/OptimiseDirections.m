function Gbest = OptimiseDirections(DirectionOptimizer, Problem, inputPopulation, evaluations)
    %% Generate random population
    Population       = inputPopulation;
    Pbest            = Population;
    [Gbest,CrowdDis] = UpdateGbest(Population,DirectionOptimizer.N);
    
    maximum = Problem.FE + evaluations;

    %% Optimization
    while Problem.FE < maximum
        Particles = [Population,Pbest,Gbest(TournamentSelection(2,DirectionOptimizer.N,-CrowdDis))];
        Population = PSO(Particles, DirectionOptimizer);
        [Gbest,CrowdDis] = UpdateGbest([Gbest,Population],DirectionOptimizer.N);
...
function NewParticles = PSO(Particles, DirectionOptimizer)
    ...
    delta    = repmat((DirectionOptimizer.upper-DirectionOptimizer.lower)/2,N/3,1);
    NewSpeed = max(min(NewSpeed,delta),-delta);
    NewDec   = ParticleDec + NewSpeed;
    
    %% Deterministic back
    Lower  = repmat(DirectionOptimizer.lower,N/3,1);
    Upper  = repmat(DirectionOptimizer.upper,N/3,1);
    ...
    NewParticles = [];
    for i = 1:N/3
        NewParticles = [NewParticles, DirectionIndividual(NewDec(i,:),DirectionOptimizer,DirectionOptimizer.directions(i,:),NewSpeed(i,:))];
    end
end